...
@scheduler.scheduled_job('interval', minutes=10, id="get_tictok")
async def main():

    bot = nonebot.get_bot()
    uid_list = bot.config.tictok
    push_groups = bot.config.push_groups

    assist_path = bot.config.assist_path
    data_path = os.path.join(os.getcwd(), assist_path, 'tictok.json')
    ...
    for uid in uid_list:
        ...
        saved_id = tictok[str(uid)]["dynamic"]["id"]
        
        # type: 1:转发    4:原创    8:视频
        if dynamic_type == 8 and dynamic_id != saved_id:
            ...
            with open(data_path, 'w') as fp:
                tictok[str(uid)]["dynamic"]["id"] = dynamic_id
                tictok[str(uid)]["dynamic"]["path"] = dynamic_path
                json.dump(tictok, fp)
        ...
        elif dynamic_type != 8 and dynamic_id != saved_id:
            dynamic_path = os.path.join(os.getcwd(), bot.config.data_path, "dynamics", "{}.png".format(dynamic_id))
            with open(data_path, 'w') as fp:
                tictok[str(uid)]["dynamic"]["id"] = dynamic_id
                tictok[str(uid)]["dynamic"]["path"] = dynamic_path
